const char* htmlPageConfig PROGMEM = R"rawliteral(
  ...
  <div id="container2" class="container">
    ...
    <p>Sélectionnez votre réseau :</p>
    <button class="button" onclick="handleRequest('vol0',function(){next(4);})">240V Monophasé</button>
    <button class="button" onclick="handleRequest('vol1',function(){next(4);})">400V Triphasés</button>
    <p class="alert">Cette configuration est cruciale lors de l'utilisation du Module TIC !</p>
  </div>
  ...
  <div id="container8" class="container">
    <h1>Configuration Wi-Fi</h1>
    <p>L'Hostname par défaut sera "SmartCharger", vous pouvez toutefois le personnaliser :</p>
    <input type="text" id="hot1" placeholder="SmartCharger">
  ...
  <div id="container15" class="container">
    ...
    <button class="button" onclick="next(16)">Oui, les personnaliser</button>
    <button class="button secondary" onclick="next(20)">Non, utiliser ceux-ci</button>
  ...
  <div id="container20" class="container">
    ...
    <button class="button" onclick="next(22)">Oui, utiliser le 80</button>
    <button class="button secondary" onclick="next(21)">Non, le modifier</button>
  ...
  <div id="container22" class="container">
    <h1>Configuration WebSocket</h1>
    <p>Le port de service WebSocket par défaut est le "443", souhaitez-vous le concerver ?</p>
    <button class="button" onclick="next(25)">Oui, utiliser le 443</button>
    <button class="button secondary" onclick="next(23)">Non, le modifier</button>
  </div>

  <div id="container23" class="container">
    <h1>Configuration WebSocket</h1>
  ...
  <div id="container25" class="container">
    <h1>Module TIC</h1>
    <p>Souhaitez-vous configurer un Module TIC déjà appairé à votre réseau ?</p>
    <button class="button" onclick="handleRequest('tic1',function(){next(26);})">Oui</button>
    <button class="button secondary" onclick="handleRequest('tic0',function(){next(30);})">Non</button>
  </div>

  <div id="container26" class="container">
    <h1>Module TIC</h1>
    <p>Renseignez les paramètres de votre Module TIC :</p>
  ...
  <div id="container30" class="container">
    <h1>Félicitation</h1>
    <p>Vous avez terminé la configuration du SmartCharger. Votre équipement doit redémarrer...</p>
  ...
  <div id="container31" class="container">
    ...
    <p class="alert">La connexion avec le SmartCharger est perdue, veuillez vous reconnecter</p>
  </div>
)rawliteral";
...
const char* scriptsPageConfig PROGMEM = R"rawliteral(
  ...
  function handleRequestAndProceed(id, value, nextContainerId) {
    if (id.startsWith('ssid')) {
      handleComplexRequest("ssi1","ssid="+encodeURIComponent(value),function(){next(nextContainerId);});
    }
    if (id.startsWith('hot1')) {
      handleComplexRequest("hot1","hot1="+encodeURIComponent(value),function(){next(nextContainerId);});
    }
    if (id.startsWith('mdp')) {
      handleComplexRequest("pas1","pass="+encodeURIComponent(value),function(){next(nextContainerId);});
    }
    if (id.startsWith('por1')) {
      handleComplexRequest("por1","por1="+encodeURIComponent(value),function(){next(nextContainerId);});
    }
    if (id.startsWith('por2')) {
      handleComplexRequest("por2","por2="+encodeURIComponent(value),function(){next(nextContainerId);});
    }
  }
